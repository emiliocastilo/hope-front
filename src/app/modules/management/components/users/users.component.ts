...
export class UsersComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.modalForm = this._formBuilder.group({
            ...
            collegeNumber: ['', [Validators.required, Validators.pattern('[0-9]{9}')]],
    ...
    public onSelectedItem(event: number): void {
        ...
        for (let i = 0; i < user_aux['rolSelected']['hospital']['services'].length; i++) {
            this.services.push(new ServiceModel(user_aux['rolSelected']['hospital']['services'][i]['id'], user_aux['rolSelected']['hospital']['services'][i]['name']));
        }

        this.hospitals.push(new HospitalModel(user_aux['rolSelected']['hospital']['id'], user_aux['rolSelected']['hospital']['name'], this.services));
        ...
        forkJoin([this.rolService.getAllRoles(this.selectedUser['rolSelected']['id'])]).subscribe((responseData) => {
            for (let i = 0; i < responseData[0].length; i++) {
                this.roles.push(new RolModel(responseData[0][i]['id'], responseData[0][i]['name'], responseData[0][i]['description']));
            }
        });
        ...
        for (let i = 0; i < this.users[event]['roles'].length; i++) {
            forkJoin([this.rolService.getRoleById(String(this.users[event]['roles'][i]))]).subscribe((responseData) => {
                this.activeRoles.push(new RolModel(responseData[0]['id'], responseData[0]['name'], responseData[0]['description']));
            });
        }
        ...
        Object.keys(this.selectedUsers).forEach((doctorKey: string) => {
            if (this.modalForm.controls[doctorKey]) {
                this.modalForm.controls[doctorKey].setValue(this.selectedUsers[doctorKey]);
            }
        });
    }
    ...
    private saveOrUpdate(event: FormGroup, modalRef: any, node: SectionModel): void {
        ...
        user.setValuesFromDinamicForm(formValues, modalRef.componentInstance.activeRoles);
