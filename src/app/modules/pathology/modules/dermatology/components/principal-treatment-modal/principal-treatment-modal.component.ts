...
export class PrincipalTreatmentModalComponent implements OnInit {
    ...
    public isDisabled(formKey) {
        if (this.type === 'suspend') {
            return ['medicine', 'family', 'atc', 'cn', 'tract', 'indication', 'dose', 'otherDose', 'regime'].indexOf(formKey) > -1;
        } else {
    ...
    public select(formKey: string, event: any) {
        if (event && formKey === 'dose') this.selectDose.emit(event);
    ...
    checkTypeTreatment(key): boolean {
        ...
        if (!this.form.get('treatmentType') && this.form.get('reasonSuspension')) {
            ...
            if (this.form.get('treatmentType') && this.form.get('treatmentType').value) {
                ...
                if (
                    (this.form.get('treatmentType').value[0] && (this.form.get('treatmentType').value[0].id === 'QUIMICO' || this.form.get('treatmentType').value[0].id === 'BIOLOGICO')) ||
                    ...
                    if (['opcionFormulaMagistral', 'opcionMedicamento', 'descripcionFormulaMagistral', 'dosisFormulaMagistral'].indexOf(key) > -1) {
                        show = false;
                    }
                    ...
                    if (this.form.get('opcionMedicamento').value === 'opcionMedicamento') {
                        if (['descripcionFormulaMagistral', 'dosisFormulaMagistral'].indexOf(key) > -1) {
                            show = false;
                        }
                        //Hemos seleccionado tratamiento topico y opcion formula magistral
                    } else if (this.form.get('opcionFormulaMagistral').value === 'opcionFormulaMagistral') {
                        if (['medicine', 'family', 'atc', 'cn', 'tract', 'dose', 'otherDosis'].indexOf(key) > -1) {
                            show = false;
                        }
                    }
                }
