...
export class EavPaseComponent implements OnInit {
    ...
    public isValidForm () {
        ...
        if (!this.form.controls['dateEvaluation2'].value && (this.form.controls['paseScoreTotal'].value || this.form.controls['valuationSymptoms'].value || this.form.controls['valuationFunctional'].value)) {
            this._notification.showErrorToast('missDate');
            return false;
        }
