...
@Component({
  selector: 'app-eritrodermia',
  templateUrl: './eritrodermia.component.html',
  styleUrls: ['./eritrodermia.component.scss'],
})
export class EritrodermiaComponent implements OnInit {
  ...
  private getData(): void {
    ...
    const chartTitle = 'patientsTreatmentEritrodermia';
    const view = null;
    const scheme = {
      domain: ['#249cf1'],
    };
  ...
  public onIconButtonClick(event: any) {
    if (event && event.type === 'detail') {
      ...
      const query = 'type=ERITRODERMIA&indication=' + this.currentSelected.name;
  ...
  public selectPage(page: number) {
    if (this.currentPage !== page) {
      ...
      const query = `type=ERITRODERMIA&indication=&page=${this.currentPage}&sort=${this.currentSort.column},${this.currentSort.direction}`;
      this.getDetails(query);
    }
  }

  public onSort(event: any) {
    const query = `type=ERITRODERMIA&indication=&page=${this.currentPage}&sort=${event.column},${event.direction}`;
