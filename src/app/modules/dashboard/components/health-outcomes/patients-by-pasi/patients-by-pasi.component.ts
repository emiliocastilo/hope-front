...
export class PatientsByPasiComponent implements OnInit {
  ...
  public columHeaders: string[] = ['resultsPASI', 'patients'];
  ...
  public onIconButtonClick(event: any): void {
    if (event.type === 'detail') {
      ...
      const query = `indexType=PASI&result=${this.currentSelected.resultsPASI}`;
  ...
  public selectPage(page: number) {
    if (this.currentPage !== page) {
      ...
      const query = `indexType=PASI&result=${this.currentSelected.resultsPASI}&page=${this.currentPage}&sort=${this.currentSort.column},${this.currentSort.direction}`;
      this.getDetails(query);
    }
  }

  public onSort(event: any) {
    const query = `indexType=PASI&result=${this.currentSelected.resultsPASI}&page=${this.currentPage}&sort=${event.column},${event.direction}`;
