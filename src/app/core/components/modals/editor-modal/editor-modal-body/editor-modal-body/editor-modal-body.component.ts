...
export class EditorModalBodyComponent implements OnInit {
    ...
    getColSection(keys: any) {
        if (this.formKeys.length >= 3 && keys !== 'roles' && keys !== 'principal' && keys !== 'active') {
        ...
        keys.forEach((key) => {
            if (this.form.controls[key].value) {
                if (key.toLowerCase().includes('date') || key.toLowerCase().includes('period')) {
                    this.form.controls[key].setValue(this.form.controls[key].value.split('T')[0]);
                }
            }
        });
    }
    ...
    onRolSelected(event: any) {
        ...
        if (this.activeRoles.length > 0) {
            ...
            if (!rolFound) {
                this.activeRoles.push(this.options.roles.options.find((rol) => rol.name === name));
            }
        } else {
            this.activeRoles.push(this.options.roles.options.find((rol) => rol.name === name));
        }
    ...
    public getRequired(formKey: string): boolean {
        const requiredFields = ['name', 'firstSurname', 'dni', 'healthCard', 'fileMedicine', 'nhc', 'birthDate'];
