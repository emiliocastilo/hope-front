...
export class PrincipalTreatmentModalComponent implements OnInit {
  ...
  public select(formKey, event) {
    ...
    if (formKey === 'treatmentType' && event.id === 'topical' || formKey === 'treatmentType' && event.id === 'biological'
  ...
  checkTypeTreatment(key): boolean{
    ...
    // TODO: comparar si estamos en cambiar o suspender, si es así, deshabilitar/habilitar los campos correspondientes
     if (this.form.get("treatmentType") && this.form.get("treatmentType").value){
      ...
      if (this.form.get("treatmentType").value[0].id === "chemical" || this.form.get("treatmentType").value[0].id === "biological"){
