...
export class ResultsPerPageComponent implements OnInit {
    ...
    calcLastPagElement(): number {
        ...
        return total < this.selectedOption ? total : pag === 1 ? this.selectedOption : total >= this.selectedOption * pag ? this.selectedOption * pag : total;
    }

    calcFirstPagElement() {
        return this.paginationData.totalElements === 0 ? 0 : this.paginationData.number === 1 ? 1 : this.selectedOption * this.paginationData.number - this.selectedOption + 1;
    }
}
